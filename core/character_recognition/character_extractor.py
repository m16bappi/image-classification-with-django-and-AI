# -*- coding: utf-8 -*-
"""
Created on Tue Sep  3 04:21:04 2019
@author: iit
"""
import csv
        ...
        for image in imgs:
            ...
            for region in regionprops(label_image):
                ...
                with open('mycsv.csv', 'a', newline='') as f:
                    ...
                    # writer.writeheader()
                    writer.writerow({'count': str(image_count) + "_" + str(image_num)})
                ...
                for char_region in regionprops(label_word_image):
                    if char_region.area > 0:
                        ...
                        if char_region.area < 10 and c_minc < 43:
                            ...
                            imageio.imwrite(
                                r"media\\" + str(image_count) + "_" + str(image_num) + "_" + str(
                                    word_image_num) + ".PNG", char_img)

                            word_image_num += 1
                            with open('mycsv_character.csv', 'a', newline='') as f:
                                field = ['count']
                                writer = csv.DictWriter(f, fieldnames=field)
                                # writer.writeheader()
                                writer.writerow(
                                    {'count': str(image_count) + "_" + str(image_num) + "_" + str(word_image_num)})
                        else:
                            continue
...
# CharacterExtractor().character_extractor(path="2.jpg")
